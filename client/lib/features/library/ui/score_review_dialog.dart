...
class _ScoreReviewDialogState extends ConsumerState<ScoreReviewDialog> {
  ...
  Future<void> _loadScores() async {
    ...
    // Sort by page number for sequential review.
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _ScoreReviewTile(
                              score: score,
                              onSave: (title, startPage) =>
                                  _saveScore(score, title, startPage),
  ...
  Future<void> _saveScore(
      ScoreModel score, String newTitle, int newStartPage) async {
    final repo = ref.read(scoreRepositoryProvider);
...
class _ScoreReviewTile extends StatefulWidget {
  ...
  final Future<void> Function(String title, int startPage) onSave;
...
class _ScoreReviewTileState extends State<_ScoreReviewTile> {
  ...
  void initState() {
    ...
    _pageController =
        TextEditingController(text: '${widget.score.startPage ?? 1}');
  }
  ...
  void didUpdateWidget(_ScoreReviewTile old) {
    ...
    if (old.score.id != widget.score.id) {
      ...
      _pageController.text = '${widget.score.startPage ?? 1}';
      _editing = false;
    }
  }
          ...
          if (_saving)
            ...
            IconButton(
              ...
              onPressed: () => setState(() {
                ...
                _pageController.text = '${score.startPage ?? 1}';
              }),
              visualDensity: VisualDensity.compact,
            ),
          ] else ...[
  ...
  Future<void> _save() async {
    ...
    if (title.isEmpty || page == null || page < 1) return;

    setState(() => _saving = true);
    await widget.onSave(title, page);
    if (mounted) {
      setState(() {
        _saving = false;
        _editing = false;
